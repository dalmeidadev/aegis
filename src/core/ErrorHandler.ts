...
export class ErrorHandler {
	...
	private async logErrorIfNeeded(
		...
		const shouldLog =
			this.logAllErrors || this.shouldLogError(config.severity || "error");
		...
		for (const adapter of this.adapters) {
			if (adapter.canHandle(error) && adapter.extractMetadata) {
				const adapterMetadata = await Promise.resolve(
					adapter.extractMetadata(error),
				);
	...
	public createQueryErrorHandler(
		...
		return async (error: unknown) => {
			...
			this.logger(`Error in query [${queryName}]: ${result.message}`, error, {
				queryName,
